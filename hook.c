...
//    skb->pkt_type = PACKET_OUTGOING;
    ...
    memcpy(skb->data, data, (int)length);
...
//                buff_tail = &buff_tail[c - 1];
                ...
                if (tmp_buff[0] == 1) {
                    send_frame(buff_head, (buff_tail - buff_head));
                    buff_tail = buff_head;
                }
//            } else {
...
//            printk(KERN_ERR "[ETHoLoRa][Transmit][DEBUG] Address of totlen is %p\n", &work_pool.data_arr[work_pool.count].total_length);
            ...
            while (total_length - sent_bytes > MAX_PACKET_SIZE - 1) {
                ...
                work_pool.data_arr[work_pool.count].data[0] = (sent_bytes == 0 ? 2
                                                                               : 0); // 2 for first, 0 for rest, 1 for last.
